...
app.patch('/api/activities/:activityId', async (req, res) => {
  try {
    ...
    const [existingActivities] = await pool.query(
      `SELECT a.id, a.half_day_id AS halfDayId, a.position, h.course_id AS courseId
       FROM activities a
       INNER JOIN half_days h ON a.half_day_id = h.id
       WHERE a.id = ?
       LIMIT 1`,
    ...
    const providedCourseId = Number(courseId);

    if (providedCourseId && providedCourseId !== activityCourseId) {
      return res.status(400).json({ error: "L'activité ne peut être modifiée pour un autre cours." });
    }
    ...
    const normalizedFormat = supportedFormats.has(format) ? format : 'presentation';
    const description = (details || '').trim() || 'Description à compléter';
    const sanitizedMaterials = materials && typeof materials === 'string' ? materials.trim() : null;
    const objective = name.trim();
    ...
    let targetPosition = existingActivity.position;

    if (existingActivity.halfDayId !== halfDay.id) {
      const [positions] = await pool.query(
        'SELECT COALESCE(MAX(position), 0) AS maxPosition FROM activities WHERE half_day_id = ?',
        [halfDay.id]
      );
      targetPosition = Number(positions[0].maxPosition) + 1;
    }

    await pool.query(
      `UPDATE activities
       SET half_day_id = ?, specific_objective = ?, description = ?, duration_minutes = ?, format = ?, materials = ?, position = ?
       WHERE id = ?`,
      [halfDay.id, objective, description, durationMinutes, normalizedFormat, sanitizedMaterials, targetPosition, activityId]
    );

    res.json({ success: true, halfDayId: halfDay.id, position: targetPosition });
  } catch (error) {
...
app.delete('/api/activities/:activityId', async (req, res) => {
  try {
    ...
    const [result] = await pool.query('DELETE FROM activities WHERE id = ?', [activityId]);
