...
app.patch('/api/activities/:activityId', async (req, res) => {
  try {
    ...
    const targetHalfDay = await getHalfDayForCourse(activityCourseId, weekNumber, slotIndex);
    if (!targetHalfDay) {
      return res.status(500).json({ error: "Impossible de déterminer le demi-jour cible." });
    }
    ...
    try {
      ...
      let newPosition = existingActivities[0].position;

      if (targetHalfDay.id !== existingActivities[0].halfDayId) {
        const [positions] = await connection.query(
          'SELECT COALESCE(MAX(position), 0) AS maxPosition FROM activities WHERE half_day_id = ?',
          [targetHalfDay.id]
        );
        newPosition = Number(positions[0].maxPosition) + 1;
      }
      ...
      await connection.query(
        `UPDATE activities
         ...
         WHERE id = ?`,
        [targetHalfDay.id, objective, description, durationMinutes, normalizedFormat, sanitizedMaterials, newPosition, activityId]
      );

      await connection.commit();

      res.json({ success: true, halfDayId: targetHalfDay.id, position: newPosition });
    } catch (error) {
      await connection.rollback();
...
app.delete('/api/activities/:activityId', async (req, res) => {
  try {
    ...
    if (courseId && courseId !== activityCourseId) {
      return res.status(400).json({ error: "L'activité ne correspond pas au cours sélectionné." });
    }
